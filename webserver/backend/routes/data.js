...
db.serialize(() => {
  db.run(
    "CREATE TABLE IF NOT EXISTS actions (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT, hostname TEXT, timestamp TEXT, content TEXT, actiontype TEXT)"
  );
  db.run(
    `CREATE TABLE IF NOT EXISTS data (id INTEGER PRIMARY KEY AUTOINCREMENT, action_id INTEGER, filename TEXT, mime_type TEXT, FOREIGN KEY (action_id) REFERENCES actions(id))`
  );
});
  ...
  db.all(`SELECT * FROM actions`, (err, actions) => {
    ...
    let actionPromises = actions.map((action) => {
      return new Promise((resolve, reject) => {
        db.all(
          `SELECT * FROM data WHERE action_id = ?`,
          [action.id],
          (err, files) => {
            if (err) {
...
router.delete("/:id", (req, res) => {
  ...
  db.all(
    `SELECT filename FROM data WHERE action_id = ?`,
    [actionId],
    (err, files) => {
      if (err) {
        console.error("Error retrieving filenames", err);
        return res.status(500).json({ message: "Internal server error" });
      }
      ...
      db.run(
        `DELETE FROM data WHERE action_id = ?`,
        [actionId],
        function (err) {
          if (err) {
            console.error("Error deleting from data table", err);
            return res.status(500).json({ message: "Internal server error" });
          }
          ...
          db.run(
            ...
            function (err) {
              if (err) {
                console.error("Error deleting from actions table", err);
                return res
                  .status(500)
              ...
              res.status(200).json({
                message: "Action and associated files deleted successfully",
              });
            }
          );
        }
